...
def handle(game, event):
    if game.state == "CHEST_REWARD":
        ...
        if event.key in [pygame.K_TAB, pygame.K_e]:
            ...
            current = tabs.index(game.inventory_tab) if hasattr(game, 'inventory_tab') and game.inventory_tab in tabs else 0
                ...
                if event.key in [pygame.K_UP, pygame.K_w]:
                    ...
                    if game.menu_index < 5:
                        ...
                        if item:
                            ...
                            game.log.add_message(f"[EQUIPO] Te has quitado: {item.nombre}.")
                            game.play_sfx("equip")
                    else:
                        game.state = "COMBAT" if game.current_enemy else "PLAYING"
            else:
                ...
                if event.key in [pygame.K_UP, pygame.K_w]:
                    ...
                    if game.menu_index < len(titulos):
                        ...
                        if t_data.get("tipo") == "pasivo":
                            game.log.add_message("[SISTEMA] Este titulo es pasivo y ya esta activo.")
                            combat.spawn_floating_text(game, "¡PASIVO YA ACTIVO!", game.player.rect.centerx, game.player.rect.top - 20, CYAN)
                        elif game.player.logic.cambiar_titulo(nuevo):
                            game.play_sfx("coins")
                            game.log.add_message(f"[TITULO] Equipado: {nuevo}")
                            combat.spawn_floating_text(game, f"★ {nuevo} ★", game.player.rect.centerx, game.player.rect.top - 20, GREEN)
                    else:
                        game.state = "COMBAT" if game.current_enemy else "PLAYING"
        ...
        if category == 'MAIN':
            ...
            if event.key == pygame.K_ESCAPE:
                ...
                if game.menu_index == 0:
                    game.open_quantity_buy("Pocion Vida Media", 100, lambda: Pocion("media"))
                elif game.menu_index == 1:
                    game.open_quantity_buy("Pocion Vida Grande", 300, lambda: Pocion("grande"))
                elif game.menu_index == 2:
                    game.open_quantity_buy("Pocion Mana Media", 150, lambda: PocionMana("media"))
                elif game.menu_index == 3:
                    game.open_quantity_buy("Pocion Regreso", 50, lambda: PocionRegreso())
                elif game.menu_index == 4:
                    if game.player.logic.gastar_monedas(10000):
                        ...
                        combat.spawn_floating_text(game, f"+{item.nombre}", game.player.rect.centerx, game.player.rect.top, YELLOW)
                    else:
                        game.log.add_message("[MERCADER] Ese libro es muy caro para ti.")
            ...
            if event.key == pygame.K_ESCAPE:
                ...
                if game.player.logic.gastar_monedas(precio):
                    game.play_sfx("coins")
                    dmg = random.randint(8 + game.player.logic.nivel, 12 + int(game.player.logic.nivel * 1.5))
                    if game.menu_index == 0:
                        item = Arma(f"Espada lvl {game.player.logic.nivel}", dmg, "fisico")
                    elif game.menu_index == 1:
                        item = Arma(f"Arco lvl {game.player.logic.nivel}", dmg, "distancia")
                    elif game.menu_index == 2:
                        item = Arma(f"Mazo lvl {game.player.logic.nivel}", dmg + 10, "contundente")
                    elif game.menu_index == 3:
                        item = Arma(f"Varita lvl {game.player.logic.nivel}", dmg - 5, "magico")
                    game.player.add_to_inventory(item)
            ...
            if event.key == pygame.K_ESCAPE:
                ...
                if game.player.logic.gastar_monedas(precio):
                    ...
                    if random.random() < 0.08: # 8% chance
                        ...
                        game.log.add_message("[!] Tu suerte despertó: +Nivel aplicado a la armadura!")
                    if game.menu_index == 0:
                        item = Armadura(f"Casco lvl {game.player.logic.nivel}", defensa, slot="casco")
                    elif game.menu_index == 1:
                        item = Armadura(f"Pechera lvl {game.player.logic.nivel}", defensa + 5, slot="pechera")
                    elif game.menu_index == 2:
                        item = Armadura(f"Botas lvl {game.player.logic.nivel}", defensa - 2, slot="botas")
                    game.player.add_to_inventory(item)
            ...
            if event.key == pygame.K_ESCAPE:
                ...
                if game.player.logic.gastar_monedas(precio):
                    ...
                    if game.menu_index == 0:
                        item = Accesorio(f"Anillo de Vida lvl {game.player.logic.nivel}", {"vida": bono*5})
                    elif game.menu_index == 1:
                        item = Accesorio(f"Amuleto de Mana lvl {game.player.logic.nivel}", {"mana": bono*5})
                    elif game.menu_index == 2:
                        item = Accesorio(f"Collar de Defensa lvl {game.player.logic.nivel}", {"defensa": bono})
                    elif game.menu_index == 3:
                        item = Accesorio(f"Anillo de Poder lvl {game.player.logic.nivel}", {"daño": bono})
                    game.player.add_to_inventory(item)
        ...
        if event.key == pygame.K_RETURN:
            if game.menu_index < len(inv):
                ...
                if hasattr(item, 'cantidad') and item.cantidad > 1:
                    ...
                    game.play_sfx("coins")
                    game.log.add_message(f"[MERCADER] Te doy {valor} Cob por {item.nombre}.")
        ...
        if event.key in [pygame.K_LEFT, pygame.K_a]:
            ...
            if game.player.logic.gastar_monedas(costo):
                ...
                game.log.add_message(f"[MERCADER] ¡Compraste {game.qty_current}x {game.qty_item_name}!")
                combat.spawn_floating_text(game, f"+{game.qty_current} {game.qty_item_name}", game.player.rect.centerx, game.player.rect.top, YELLOW)
            else:
        ...
        if event.key in [pygame.K_LEFT, pygame.K_a]:
            ...
            game.play_sfx("coins")
            game.log.add_message(f"[MERCADER] Vendiste {game.qty_current}x {game.qty_item_name} por {ganancia} Cob.")
            combat.spawn_floating_text(game, f"+{ganancia} Cob", game.player.rect.centerx, game.player.rect.top, (205, 127, 50))
        ...
        if event.key == pygame.K_UP or event.key == pygame.K_w:
            ...
            if game.menu_index == 0:
                game.save_files = SaveManager.get_save_files()
                if getattr(game, "current_save_file", None) is None and len(game.save_files) >= 5:
                    game.state = "SAVE_SELECTION"
                    game.save_return_state = "CROSS_MENU"
                    game.menu_index = 0
                    game.log.add_message("[SISTEMA] Limite de guardados. Selecciona uno para sobrescribir o 'D' para borrar.")
                else:
                    save_success = SaveManager.save_game(game, getattr(game, "current_save_file", None))
                    if save_success:
                        game.log.add_message("[SISTEMA] Partida guardada correctamente.")
                ...
                if game.player.logic.cruz_usos_hoy > 0:
                    ...
                    game.log.add_message(f"[CRUZ] Rezas con devocion. (Usos restantes: {game.player.logic.cruz_usos_hoy})")
                    combat.spawn_floating_text(game, "¡Vida y Mana al Max!", game.player.rect.centerx, game.player.rect.top - 20, CYAN)
                else:
                    game.log.add_message("[CRUZ] No tienes mas usos de oracion en este dia de juego.")
            elif game.menu_index == 2:
                if game.player.logic.cruz_usos_hoy == 3:
                    ...
                    game.log.add_message("[CRUZ] Cuestionas tu fe. Has quedado penalizado y no podras curarte hoy en la Cruz.")
                    combat.spawn_floating_text(game, "¿Fe cuestionada? (Sin curacion)", game.player.rect.centerx, game.player.rect.top - 20, RED)
                    
                    game.player.logic.verificar_titulos(game.log)
                else:
                    game.log.add_message("[CRUZ] Debes tener los 3 usos de oracion del dia intactos para cuestionar tus creencias.")
            elif game.menu_index == 3:
                game.state = "PLAYING"
