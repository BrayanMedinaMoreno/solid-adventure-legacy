...
class Chest(pygame.sprite.Sprite):
    def __init__(self, game, x, y):
        ...
        try:
            ...
            pygame.draw.rect(
                self.image,
                YELLOW,
                (10, TILESIZE // 2, TILESIZE - 20, TILESIZE // 2 - 10),
            )
    ...
    def open(self):
        ...
        if random.random() < 0.4:  # 40% de probabilidad de ser poción
            tipo_p = random.choices(
                ["pequeña", "media", "grande"], weights=[80, 15, 5]
            )[0]
            ...
            nombres_arma = {
                "guerrero": {
                    "Espada Oxidada": "fisico",
                    "Daga Rápida": "fisico",
                    "Hacha Pesada": "contundente",
                    "Mazo de Guerra": "contundente",
                },
                "tirador": {
                    "Pistola Vieja": "distancia",
                    "Rifle de Caza": "distancia",
                    "Ballesta": "distancia",
                    "Escopeta Recortada": "distancia",
                },
            }
            nombres_armadura = {
            ...
            clase_p = random.choice(list(nombres_arma.keys()))
            ...
            if rand_gear < 0.2:  # 20% Accesorio
                ...
                if stat in ["max_vida", "max_mana"]:
                    val *= 5
                item = Accesorio(random.choice(nombres_acc), {stat: val})
            elif rand_gear < 0.7:
                tabla = nombres_arma[clase_p]
                nombre_arma = random.choice(list(tabla.keys()))
                tipo = tabla[nombre_arma]
                item = Arma(nombre_arma, daño_def, tipo)
            else:  # 30% Armadura
                item = Armadura(
                    random.choice(nombres_armadura[clase_p]), daño_def // 2 + 2, clase_p
                )
