...
class Game:
    ...
    def open_quantity_buy(self, item_name, unit_price, item_factory):
        ...
        if max_posible < 1:
            ...
            self.spawn_floating_text(
                "¡SIN DINERO!", self.player.rect.centerx, self.player.rect.top - 20, RED
            )
            return
        self.qty_mode = "BUY"
    ...
    def start_game(self, clase_elegida, nombre=None, dificultad="normal"):
        ...
        self.log.add_message(
            f"Dificultad: {'Fácil' if dificultad == 'facil' else 'Normal'}"
        )
    ...
    def load_level(self):
        ...
        if not self.player:
            ...
            if self.profundidad == 0:
                start_x, start_y = (
                    self.level.entrance
                    if self.level.entrance
                    else (self.level.width_tiles // 2, self.level.height_tiles // 2 + 2)
                )
            elif self.went_down:
                start_x, start_y = (
                    self.level.entrance
                    if self.level.entrance
                    else self.level.floor_tiles[0]
                )
            else:
                start_x, start_y = (
                    self.level.stairs_down
                    if self.level.stairs_down
                    else self.level.floor_tiles[0]
                )
        ...
        if self.level.stairs_down:
            occupied_tiles.add(self.level.stairs_down)
    ...
    def update(self):
        # Actualizar lógica
        if hasattr(self, "player") and self.player:
            self.player.logic.tiempo_juego = (
                getattr(self.player.logic, "tiempo_juego", 0.0) + self.dt
            )
        ...
        if (
            hasattr(self, "music_volume_display_timer")
            and self.music_volume_display_timer > 0
        ):
    ...
    def draw(self):
        ...
        for sprite in self.all_sprites:
            ...
            if sprite.rect.colliderect(screen_rect):
                ...
                if self.profundidad > 0 and sprite != self.player:
                    ...
                    if (
                        0 <= grid_x < self.level.width_tiles
                        and 0 <= grid_y < self.level.height_tiles
                    ):
            ...
            if enemy.rect.colliderect(screen_rect):
                ...
                if self.profundidad > 0:
                    ...
                    if not (
                        0 <= grid_x < self.level.width_tiles
                        and 0 <= grid_y < self.level.height_tiles
                        and self.level.explored[grid_y][grid_x]
                    ):
                        continue
                ...
                outline_rect = pygame.Rect(
                    enemy.rect.x - cam_x + 4,
                    enemy.rect.y - cam_y - 10,
                    bar_width,
                    bar_height,
                )
        ...
        if self.state == "COMBAT":
            ...
            self.draw_bank_menu()
        elif self.state in ["HERRERO", "HERRERO_ACTION"]:
            self.draw_herrero_menu()
    ...
    def draw_herrero_menu(self):
        game_screens.draw_herrero_menu(self)

    def draw_bank_store_menu(self):
        import screens.game_screens as game_screens

        game_screens.draw_vault_menu(
            self, self.player.inventory, "BAÚL (GUARDAR OBJETOS)"
        )
    ...
    def draw_bank_retrieve_menu(self):
        ...
        game_screens.draw_vault_menu(
            self, self.player.logic.baul, "BAÚL (RETIRAR OBJETOS)"
        )
    ...
    def draw_vault_menu(self, items, title):
        ...
        for i in range(
            self.vault_scroll, min(len(items), self.vault_scroll + max_visible)
        ):
            item = items[i]
