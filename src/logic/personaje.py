...
TITULOS_DATA = {
    ...
    "Arquero de Élite": {
        ...
        "req": {
            "usos_proyectil": 1000,
            "muertes_slime": 100,
            "titulos": ["Iniciado en Proyectiles"],
        },
    ...
    "Esquiva Intermedio": {
        ...
        "bono_pasivo": {
            "esquiva_basica": 0.06,
            "esquiva_trampa": 0.20,
            "esquiva_distancia": 0.02,
            "esquiva_magica": 0.02,
            "esquiva_habilidad": 0.02,
        },
    },
    "Esquiva de Veterano": {
        "tipo": "pasivo",
        ...
        "bono_pasivo": {
            "esquiva_basica": 0.08,
            "esquiva_trampa": 0.30,
            "esquiva_distancia": 0.04,
            "esquiva_magica": 0.04,
            "esquiva_habilidad": 0.04,
        },
    },
    "Esquiva de Experto": {
        "tipo": "pasivo",
    ...
    "Último Aliento Iniciado": {
        ...
        "req": {
            "combates_bajo_20hp": 100,
            "aliento_activaciones_novato": 60,
            "titulos": ["Último Aliento Novato"],
        },
    ...
    "Último Aliento Intermedio": {
        ...
        "req": {
            "combates_bajo_25hp": 150,
            "aliento_activaciones_iniciado": 60,
            "titulos": ["Último Aliento Iniciado"],
        },
    ...
    "Último Aliento Veterano": {
        ...
        "req": {
            "combates_bajo_25hp": 200,
            "aliento_activaciones_intermedio": 80,
            "titulos": ["Último Aliento Intermedio"],
        },
    ...
    "Último Aliento Experto": {
        ...
        "req": {
            "combates_bajo_25hp": 250,
            "aliento_activaciones_veterano": 80,
            "titulos": ["Último Aliento Veterano"],
        },
    ...
    "Último Aliento de Maestro": {
        ...
        "req": {
            "combates_bajo_25hp": 350,
            "aliento_activaciones_experto": 100,
            "titulos": ["Último Aliento Experto"],
        },
    ...
    "Regeneración Pasiva Intermedia": {
        ...
        "req": {
            "hp_regenerada_total": 10000,
            "regen_activaciones_baja": 100,
            "titulos": ["Regeneración Pasiva Baja"],
        },
    ...
    "Regeneración Pasiva Media": {
        ...
        "req": {
            "hp_regenerada_total": 25000,
            "regen_activaciones_intermedia": 150,
            "titulos": ["Regeneración Pasiva Intermedia"],
        },
    ...
    "Regeneración Pasiva Alta": {
        ...
        "req": {
            "hp_regenerada_total": 60000,
            "regen_activaciones_media": 200,
            "titulos": ["Regeneración Pasiva Media"],
        },
    ...
    "Regeneración Pasiva Superior": {
        ...
        "req": {
            "hp_regenerada_total": 150000,
            "regen_activaciones_alta": 300,
            "titulos": ["Regeneración Pasiva Alta"],
        },
...
class Personaje:
    ...
    def fuerza(self):
        bono = sum(
            TITULOS_DATA[t]["bono"].get("fuerza", 0)
            for t in self.titulos_activos
            if t in TITULOS_DATA and "bono" in TITULOS_DATA[t]
        )
    ...
    def defensa(self):
        bono = sum(
            TITULOS_DATA[t]["bono"].get("defensa", 0)
            for t in self.titulos_activos
            if t in TITULOS_DATA and "bono" in TITULOS_DATA[t]
        )
        def_equipo = 0
        if self.casco:
            def_equipo += self.casco.defensa
        if self.pechera:
            def_equipo += self.pechera.defensa
        if self.botas:
            def_equipo += self.botas.defensa
    ...
    def magia(self):
        bono = sum(
            TITULOS_DATA[t]["bono"].get("magia", 0)
            for t in self.titulos_activos
            if t in TITULOS_DATA and "bono" in TITULOS_DATA[t]
        )
    ...
    def defensa_magica(self):
        bono = sum(
            TITULOS_DATA[t]["bono"].get("defensa_magica", 0)
            for t in self.titulos_activos
            if t in TITULOS_DATA and "bono" in TITULOS_DATA[t]
        )
        def_eq = 0
        if self.casco:
            def_eq += getattr(self.casco, "defensa_magica", 0)
        if self.pechera:
            def_eq += getattr(self.pechera, "defensa_magica", 0)
        if self.botas:
            def_eq += getattr(self.botas, "defensa_magica", 0)
        return self.defensa_magica_base + bono + def_eq

    @property
    def max_vida(self):
        bono = sum(
            TITULOS_DATA[t]["bono"].get("max_vida", 0)
            for t in self.titulos_activos
            if t in TITULOS_DATA and "bono" in TITULOS_DATA[t]
        )
        ...
        bono_pasivo_extra = sum(
            (
                TITULOS_DATA[t].get("bono_pasivo_oculto", {}).get("max_vida", 0)
                for t in self.titulos_activos
    ...
    def max_mana(self):
        bono = sum(
            TITULOS_DATA[t]["bono"].get("max_mana", 0)
            for t in self.titulos_activos
            if t in TITULOS_DATA and "bono" in TITULOS_DATA[t]
        )
    ...
    def recibir_daño(self, dmg, tipo="fisico", log=None):
        ...
        bono_equipado = sum(
            TITULOS_DATA[t].get("bono_pasivo_oculto", {}).get("esquiva_basica", 0)
            for t in self.titulos_activos
            if t in TITULOS_DATA
        )
        ...
        if random.random() < prob_esquiva:
            if log:
                log.add_message(f"[{self.nombre}] ¡ESQUIVADO!")
            if self.game:
            ...
            if "Esquiva de Novato" in self.titulos_desbloqueados:
                self.acciones["esquivas_novato"] += 1
            if "Esquiva de Iniciado" in self.titulos_desbloqueados:
                self.acciones["esquivas_iniciado"] += 1
            if "Esquiva Intermedio" in self.titulos_desbloqueados:
                self.acciones["esquivas_intermedio"] += 1
            if "Esquiva de Veterano" in self.titulos_desbloqueados:
                self.acciones["esquivas_veterano"] += 1
            if "Esquiva de Experto" in self.titulos_desbloqueados:
                self.acciones["esquivas_experto"] += 1

            if tipo == "trampa":
                self.acciones["trampas_esquivadas"] += 1
        ...
        self.min_porcentaje_vida_combate = min(
        ...
        if not self.aliento_usado_combate and self.vida > 0:
            ...
            if self.vida <= mejor_trigger:
                ...
                if log:
                    log.add_message(f"[TITULO] ¡ÚLTIMO ALIENTO! +{curacion} HP")
                if self.game:
                    self.game.spawn_floating_text(
                        "LAST STAND",
                        self.game.player.rect.centerx,
                ...
                if "Último Aliento Novato" in self.titulos_desbloqueados:
                    self.acciones["aliento_activaciones_novato"] += 1
                if "Último Aliento Iniciado" in self.titulos_desbloqueados:
                    self.acciones["aliento_activaciones_iniciado"] += 1
                if "Último Aliento Intermedio" in self.titulos_desbloqueados:
                    self.acciones["aliento_activaciones_intermedio"] += 1
                if "Último Aliento Veterano" in self.titulos_desbloqueados:
                    self.acciones["aliento_activaciones_veterano"] += 1
                if "Último Aliento Experto" in self.titulos_desbloqueados:
                    self.acciones["aliento_activaciones_experto"] += 1
    ...
    def daño(self, oponente, tipo="fisico") -> tuple[int, bool]:
        if tipo == "magico":
            ...
            for t in getattr(self, "titulos_activos", []):
                if t in TITULOS_DATA:
                    ataque_total += (
                        TITULOS_DATA[t].get("bono", {}).get("daño_magico", 0)
                    )
    ...
    def atacar(self, oponente, log, tipo_forzado=None):
        ...
        if tipo_forzado:
            ...
            if (
                tipo_forzado == "fisico"
                and self.arma
                and self.arma.tipo_daño == "distancia"
            ):
                tipo_daño = "distancia"
        else:
            ...
            for t in getattr(self, "titulos_activos", []):
                if "Espada" in t or "Hoja" in t:
                    tipo_daño = (
                        "habilidad"  # Convertir ataques básicos en ataques de clase
                    )
        ...
        for titulo, data in TITULOS_DATA.items():
            ...
            for req_key, req_val in data["req"].items():
                if req_key == "nivel":
                    ...
                    if not self.magia_desbloqueada:
                        cumple = False
    ...
    def ejecutar_regen_turno(self, log=None):
        ...
        if regen_flat > 0:
            self.curar(regen_flat)
            if log:
                log.add_message(f"[TITULO] Regeneración: +{regen_flat} HP")
            if self.game:
                self.game.spawn_floating_text(
                    f"+{regen_flat}",
                    self.game.player.rect.centerx,
        ...
        if random.random() < regen_chance:
            ...
            self.curar(curacion)
            if log:
                log.add_message(f"[TITULO] ¡ABSORCIÓN DE ESENCIA! +{curacion} HP")
            if self.game:
                self.game.spawn_floating_text(
                    "ESENCIA",
                    self.game.player.rect.centerx,
            ...
            # Incrementar activaciones para evolución
            if "Regeneración Pasiva Baja" in self.titulos_desbloqueados:
                self.acciones["regen_activaciones_baja"] += 1
            if "Regeneración Pasiva Intermedia" in self.titulos_desbloqueados:
                self.acciones["regen_activaciones_intermedia"] += 1
            if "Regeneración Pasiva Media" in self.titulos_desbloqueados:
                self.acciones["regen_activaciones_media"] += 1
            if "Regeneración Pasiva Alta" in self.titulos_desbloqueados:
                self.acciones["regen_activaciones_alta"] += 1
    ...
    def load_base_stats(self, data):
        ...
        if data.get("casco"):
            self.casco = Armadura.from_dict(data["casco"])
        if data.get("pechera"):
            self.pechera = Armadura.from_dict(data["pechera"])
        if data.get("botas"):
            self.botas = Armadura.from_dict(data["botas"])
        if data.get("armadura"):
            self.pechera = Armadura.from_dict(data["armadura"])  # Compatibilidad
        if data.get("accesorio"):
            self.accesorio = Accesorio.from_dict(data["accesorio"])
